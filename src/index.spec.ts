...
describe('createApp', () => {
  ...
  it('mesma função atende todas as rotas /me e valida autenticação', () => {
    ...
    expect(unauthJsonMock).toHaveBeenCalledWith({
  ...
  it('protege rotas /me com autorização de ADMIN ou CLIENT', () => {
    ...
    expect(
      getMock.mock.calls.find((call) => call[0] === '/users/me')?.[1],
    ).toBe(secondAuthorizeResult);
    expect(
      getMock.mock.calls.find((call) => call[0] === '/clientes/me')?.[1],
    ).toBe(thirdAuthorizeResult);
  ...
  it('protege rotas administrativas com authorize e registra seus módulos', () => {
    ...
    expect(authorizeMock).toHaveBeenNthCalledWith(6, [
      mockUserRole.ADMIN,
      mockUserRole.CLIENT,
    ]);
    expect(authorizeMock).toHaveBeenNthCalledWith(7, [
      mockUserRole.ADMIN,
      mockUserRole.CLIENT,
    ]);
    expect(authorizeMock).toHaveBeenNthCalledWith(8, [
      mockUserRole.ADMIN,
      mockUserRole.CLIENT,
    ]);

    const dashboardCall = useMock.mock.calls.find(
      (call) => call[0] === '/dashboard',
    );
    const clientesCall = useMock.mock.calls.find(
      (call) => call[0] === '/clientes',
    );
    const veiculosCall = useMock.mock.calls.find(
      (call) => call[0] === '/veiculos',
    );
    ...
    const servicosCall = useMock.mock.calls.find(
      (call) => call[0] === '/servicos',
    );
